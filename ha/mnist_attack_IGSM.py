...
class Sample()
    ...
    self.mean =    np.zeros(n_sz)
    self.m_var = np.var(noise)
    self.results_a = np.zeros([10,n_iter])
    self.results_o = np.zeros([10,n_iter])
    self.results_c = np.zeros([10,n_iter])
    self.results_z = np.zeros([10,n_iter])

    self.n_iter = n_iter # number of iterations to try
    self.n_real = n_real # number of realizations for each iteration
    self.v_scal = v_scal # maximum sample variance : iterations will
                         #     linearly interpolate from 0 to v_scal
    self.samp = np.random.multivariate_normal(mean, cov, n_real)
...
# TODO: plot confidences instead of sample counts
# TODO: plot confidences along FGSM gradient
# TODO: plot decay in confidences along IGSM gradient steps.
# TODO: Plot 5k samples along the vector to the adversarial point (see how it changes)
# TODO: Keep 5kx300 samples for reuse during each plot
