...
if (os.path.isfile(sfile)):
  ...
  adir = "C:/Users/Nexus/Desktop/adversarial_examples"
  tdir = "C:/Users/Nexus/Desktop/torchModelZoo/"  
  ...
  adir = "/home/bwbell/Desktop/adversarial_examples"
  tdir = "/home/bwbell/Desktop/torchModelZoo/"  
...
def bracketting (image, net, n, tol, n_real):
  ...
  a_var0 = np.var(image.detach().numpy())/4
  a_var = a_var0
  l_var = 0
  u_var = a_var*2
  a_vars = np.zeros(n)
...
os.environ['TORCH_MODEL_ZOO'] = tdir
...
# c_cor = ImageNet_mapping[str(il_dict[i_name][0])]
# print("Checking image {} \n\tcorrect class: {}".format(i_name, c_cor))
def correct(x, i_name):
  x = Variable(image_loader(ddir+"/"+i_name),requires_grad=True)
  output = vgg16.forward(xs)
 
		# predicted class
  y_p = Variable(torch.LongTensor(np.array([output.data.numpy().argmax()])), requires_grad = False).numpy()[0]
# 		# true class
  y_t = il_dict[i_name][0]
  if (y_p != y_t):
    print("Ommitting {} incorrectly guessed as {} instead of {}".format(i_name, y_p, y_t))
  return(y_p == y_t)
...
#   k_brackets[i] = bracketting(xs,model, n_iter, .68, n_real)
...
for a_class in range(0,5):
  ...
  for a_img in a_imgs:
    ...
    for i in range(0,n_crop):
      image = Image.open(ddir+"/"+a_img)
      xs = Variable(image_loader_crop(image),requires_grad=True)    
      # check that image is classified correctly
      if correct(xs, a_img):
        a = bracketting(xs,model, n_iter, .68, n_real)
        b3[a_img].append(a)
        b = a["a_var"]
        b3v[a_img].append(b)
  ...
  with open(fo, 'wb') as handle:
      pickle.dump(b3vv, handle, protocol=pickle.HIGHEST_PROTOCOL)
  with open(fo, 'rb') as handle:
      b_test = pickle.load(handle)
...
baseDirectory = adir#'./adversarial_examples' 
...
for classDir in glob(baseDirectory + '/*'):
    aclass = int(classDir[-1])
    fo = odir+"/"+str(aclass)+"-vars.pickle"
    with open(fo, 'rb') as handle:
        b_test = pickle.load(handle)
	...
	#plt.imshow(img_np.swapaxes(0,1).swapaxes(1,2))        
        ...
        test = bracketting(img, model, n_iter, .68, n_real)
        test_count = sum(b_test < test["a_var"])
        test_thres = test_count/len(b_test)
        tcount+=1
        if (test_thres < t_hold):
          print("sample_bracket:{}, test_thres:{}/{}".format(test["a_var"],test_count,len(b_test)))
          scount+= 1
        ...
        # plt.show()
        if (tcount >= ncount):
          print("Correctly spotted {}/{}".format(scount, tcount))
          break

fo = odir+"/"+str(aclass)+"-results.txt"
fof = open(fo, "w")
fof.write("count: {}/{}".format(scount,tcount))
fof.close()
