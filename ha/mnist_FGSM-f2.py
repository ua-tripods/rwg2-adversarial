...
conf = [4*28*28, 4*100, 4*25, 4*10, 10]
net = Net(conf)
...
sw = 28
# Reshape
ims2 = [im.reshape(sl,sw) for im in images]
# convert to Image
ims3 = [Image.fromarray(im) for im in ims2]
# rescale to sfactor

ims4 = [im.resize((sf*sw,sf*sl), Image.NEAREST) for im in ims3]
...
#plt.imshow(ims5)
#plt.show()
ims6 = [im.reshape(sf*sl*sf*sw) for im in ims5]
